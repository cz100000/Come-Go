...
const VERIFIED_CURRENT_DAYS={"2026-08-04":{"date":"2026-08-04","entries":[{"type":"in","actual":"11:40","logged":"11:40","source":"manual","createdAt":"2026-08-04T15:53:17.154Z","edited":true},{"type":"out","actual":"17:50","logged":"17:50","source":"manual","edited":true,"createdAt":"2026-08-04T16:04:06.868Z"}],"pauseMinutes":0,"absence":null,"absenceCode":null,"note":"","holiday":null,"sourceYear":2026,"archived":false,"excelNetMinutes":0,"excelTargetMinutes":480,"excelDiffMinutes":-480,"excelBalanceMinutes":7311,"excelIncludedInSummary":true,"importDataVersion":4,"absenceNote":"","capturedAfterImport":true,"modifiedAt":"2026-08-04T16:04:10.894Z","edited":true}};Object.entries(VERIFIED_CURRENT_DAYS).forEach(([date,verified])=>{const current=migrated.days[date];const hasEntries=Array.isArray(current?.entries)&&current.entries.length>0;const hasNewerLocal=!!current?.modifiedAt&&current.modifiedAt>verified.modifiedAt;if(!hasEntries&&!hasNewerLocal)migrated.days[date]=clone(verified)});
...
renderWorkdayIssueIndicators();renderTodayCapture(d);updateCountdown();
}
...
const entries=d.entries||[],count=entries.length;$('todayBookingCount').textContent=`${count} ${count===1?'Eintrag':'Einträge'}`;
const rows=entries.map((entry,index)=>{
const isFirst=index===0,isLast=index===entries.length-1;
let label;if(entry.type==='in')label=isFirst?'Kommen':'Pause Ende';else label=isLast&&entries.length%2===0?'Gehen':'Pause';
const documented=entry.logged||roundLogged(entry.actual,entry.type),actual=entry.actual||documented,showActual=actual&&documented&&actual!==documented;
return `<button type="button" class="today-timeline-row ${entry.type}" onclick="openTodayInTimes()" aria-label="${esc(label)} um ${esc(documented)} unter Zeiten öffnen"><span class="today-timeline-line" aria-hidden="true"><i></i></span><span class="today-timeline-time">${esc(documented)}</span><span class="today-timeline-copy"><b>${esc(label)}</b>${showActual?`<small>Tatsächlich ${esc(actual)}</small>`:''}</span><span class="today-timeline-chev" aria-hidden="true">›</span></button>`;
}).join('');
const empty=!entries.length?`<button type="button" class="today-timeline-empty" onclick="openTodayInTimes()"><span><b>Noch keine Buchungen</b><small>${esc(hasFullAbsence(d)?'Für die heutige Abwesenheit sind keine Arbeitszeitbuchungen erforderlich.':'Starte den Tag mit „Kommen“.')}</small></span><span aria-hidden="true">›</span></button>`:'';
...
function renderTimes(){renderWorkdayIssueIndicators();if(currentView==='day')renderDayView(dateKey(cursorDate));else if(currentView==='week')renderWeekView(dateKey(cursorDate));else if(currentView==='month'){updateTimesWeekendControl(false);renderMonthOverview()}else{updateTimesWeekendControl(false);renderYearOverview()}updateDayQuickButton()}
...
quickContextDate=date;$('quickAddTitle').textContent='Eintrag hinzufügen';$('quickAddContext').textContent=formatContextDate(date);openModal('quickAddModal');
}
...
function confirmWorkdayIssue(date){const issue=workdayIssueForDate(date,{includeReviewed:true});if(!issue){showToast('Für diesen Tag besteht kein offener Prüfhinweis mehr.');refreshAllDerivedViews();return}if(!confirm('Der Tag wurde geprüft. Die angezeigten Minusstunden sind korrekt und es sind keine weiteren Eingaben erforderlich?'))return;const d=dayObject(date,true);d.workdayIssueReview={signature:issue.signature,reviewedAt:new Date().toISOString(),label:'Geprüft – Minusstunden sind korrekt'};state.days[date]=d;touchDay(date);closeModal('workdayIssuesModal');refreshAllDerivedViews();showToast('Arbeitstag als geprüft bestätigt. Der Zähler unter „Zeiten“ wurde aktualisiert.')}
function renderWorkdayIssueIndicators(){
const issues=workdayIssues(),count=issues.length,badge=$('timesOpenBadge'),row=$('timesOpenWorkdays');
if(badge){badge.hidden=!count;badge.textContent=count>99?'99+':String(count);badge.setAttribute('aria-label',`${count} ${count===1?'offener Arbeitstag':'offene Arbeitstage'}`)}
if(row){row.hidden=!count;if(count)$('timesOpenWorkdaysTitle').textContent=`${count} ${count===1?'offener Arbeitstag':'offene Arbeitstage'}`}
}
function renderPastWorkdayNotice(){renderWorkdayIssueIndicators()}
function openWorkdayIssues(){
const issues=workdayIssues(),missing=issues.filter(issue=>issue.kind==='missing').length,incomplete=issues.length-missing;$('workdayIssuesSummary').textContent=issues.length?`${issues.length} ${issues.length===1?'offener Arbeitstag':'offene Arbeitstage'} · ${missing} ohne Buchung · ${incomplete} unvollständig`:'Keine offenen Arbeitstage';$('workdayIssuesList').innerHTML=issues.map(issue=>`<article><div><b>${formatContextDate(issue.date)}</b><span>${esc(issue.status)}</span></div><div><button type="button" class="issue-primary-button" onclick="openIssueDay('${issue.date}')">Tag öffnen</button><button type="button" onclick="openIssueAbsence('${issue.date}')">Abwesenheit</button><button type="button" class="issue-reviewed-button" onclick="confirmWorkdayIssue('${issue.date}')">Als geprüft bestätigen</button></div></article>`).join('')||'<div class="empty">Keine offenen Arbeitstage.</div>';openModal('workdayIssuesModal')
}
function openIssueDay(date){closeModal('workdayIssuesModal');openFullDayForDate(date)}
function openIssueTime(date){openIssueDay(date)}
...
let absenceToUserAdjusted=false;
function prepareAbsenceEndDate({force=false}={}){
  const from=$('absenceFrom'),to=$('absenceTo');if(!from||!to)return;
  const start=from.value;to.min=start||TRACKING_START_DATE;
  if(start&&(force||!absenceToUserAdjusted||!to.value||to.value<start))to.value=start;
}
function onAbsenceFromChanged(){prepareAbsenceEndDate();updateAbsenceSummary()}
function onAbsenceToChanged(){const from=$('absenceFrom').value,to=$('absenceTo');absenceToUserAdjusted=true;to.min=from||TRACKING_START_DATE;if(from&&(!to.value||to.value<from))to.value=from;updateAbsenceSummary()}
function prepareAbsenceToPicker(){const from=$('absenceFrom').value,to=$('absenceTo');if(!to)return;to.min=from||TRACKING_START_DATE;if(from&&(!to.value||to.value<from))to.value=from}
function openNewAbsence(code='vacation',date=todayKey()){
['quickAddModal','absenceTypeModal','quickAbsenceModal','vacationManagerModal'].forEach(id=>{if($(id)?.classList.contains('open'))closeModal(id)});absenceEditorContext={mode:'new',scope:'range',originalGroupId:null,sourceDate:date,originalDates:[]};
$('absenceModalTitle').textContent='Abwesenheit eintragen oder bearbeiten';setAbsenceType(code);absenceToUserAdjusted=false;$('absenceFrom').value=date;$('absenceTo').value=date;prepareAbsenceEndDate({force:true});$('absenceExtent').value='full';$('absenceNote').value='';$('absenceConflictPolicy').value='abort';$('absenceDeleteActions').hidden=true;updateAbsenceSummary();openModal('absenceModal');
}
...
$('absenceModalTitle').textContent='Abwesenheit eintragen oder bearbeiten';setAbsenceType(d.absenceCode||absenceCodeFromLabel(d.absence));absenceToUserAdjusted=from!==to;$('absenceFrom').value=from;$('absenceTo').value=to;prepareAbsenceEndDate();$('absenceExtent').value=absenceDuration(d);$('absenceNote').value=d.absenceNote||'';$('absenceConflictPolicy').value='abort';$('absenceDeleteActions').hidden=false;$('deleteAbsenceGroupBtn').hidden=!(d.absenceGroupId&&absenceGroupDays(d.absenceGroupId).length>1);updateAbsenceSummary();openModal('absenceModal');
}
...
if(extent==='half'&&from!==to)return{error:'Ein halber Tag ist nur bei einem einzelnen Datum möglich.'};
const range=dateRange(from,to),workdays=range.filter(isAbsenceWorkday),originalDates=new Set(absenceEditorContext?.originalDates||[]),conflicts=workdays.filter(k=>!originalDates.has(k)&&absenceConflict(k,null)),total=workdays.reduce((n,k)=>{const base=scheduledTargetMinutes(k);return n+(code==='timeOff'?base:(extent==='half'?Math.round(base/2):0))},0);
return{from,to,range,workdays,conflicts,total,code,extent};
}
function updateAbsenceSummary(){
const box=$('absenceSummary'),from=$('absenceFrom').value,to=$('absenceTo').value,halfOption=$('absenceExtent').querySelector('option[value="half"]'),multi=!!(from&&to&&from!==to);halfOption.disabled=multi;if(multi&&$('absenceExtent').value==='half')$('absenceExtent').value='full';
const plan=absencePlan(),policyField=$('absenceConflictPolicyField');syncAbsenceExistingNotice();if(plan.error){policyField.hidden=true;box.innerHTML=`<b>Eingaben prüfen</b><span>${esc(plan.error)}</span>`;$('saveAbsenceBtn').disabled=true;return}
...
function createExcelFile(stamp=backupTimestamp(),exportedAt=new Date()){return new File([makeWorkbook()],`Arbeitszeit_Auswertung_${stamp}.xlsx`,{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',lastModified:exportedAt.getTime()})}
...
async function createPackage(){const now=new Date(),stamp=backupTimestamp(now);return[createBackupFile(stamp,now),createExcelFile(stamp,now)]}
...
const prefix=`${year}-`,today=todayKey(),days=Object.values(state.days||{}).filter(d=>d.date?.startsWith(prefix)&&dayAbsenceCode(d)==='vacation').sort((a,b)=>a.date.localeCompare(b.date)),groups=[],byId=new Map();let legacy=null;
const canContinue=(prev,current)=>{if(!prev||absenceDuration(prev)!==absenceDuration(current)||normalizeNoteText(prev.absenceNote)!==normalizeNoteText(current.absenceNote)||absenceDuration(current)==='half')return false;let cursor=addDays(parseDateKey(prev.date),1);while(cursor<current.date&&scheduledTargetMinutes(cursor)===0)cursor=addDays(parseDateKey(cursor),1);return cursor===current.date};
for(const d of days){if(d.absenceGroupId){if(!byId.has(d.absenceGroupId)){const arr=[];byId.set(d.absenceGroupId,arr);groups.push(arr)}byId.get(d.absenceGroupId).push(d);legacy=null;continue}if(legacy&&canContinue(legacy.at(-1),d))legacy.push(d);else{legacy=[d];groups.push(legacy)}}
return groups.map((items,index)=>{const sorted=items.sort((a,b)=>a.date.localeCompare(b.date)),units=sorted.reduce((n,d)=>n+(absenceDuration(d)==='half'?0.5:1),0),takenUnits=sorted.filter(d=>d.date<=today).reduce((n,d)=>n+(absenceDuration(d)==='half'?0.5:1),0),plannedUnits=units-takenUnits,status=takenUnits&&plannedUnits?'mixed':plannedUnits?'planned':'taken';return{key:sorted[0].absenceGroupId||`legacy:${year}:${index}:${sorted[0].date}`,from:sorted[0].date,to:sorted.at(-1).date,units,takenUnits,plannedUnits,status,note:sorted.find(d=>normalizeNoteText(d.absenceNote))?.absenceNote||'',days:sorted}}).sort((a,b)=>a.from.localeCompare(b.from));
}
function vacationSummary(year){
const records=vacationRecordsForYear(year),ent=vacationEntitlementForYear(year);let taken=0,planned=0;for(const r of records){taken+=r.takenUnits;planned+=r.plannedUnits}const total=ent.days+ent.carryover;return{records,ent,total,taken,planned,remaining:total-taken-planned};
}
...
const yearSel=$('vacationYear');if(!yearSel)return;const selected=Number(yearSel.value)||new Date().getFullYear();yearSel.innerHTML=vacationYearOptions().map(y=>`<option value="${y}">${y}</option>`).join('');yearSel.value=String(selected);const s=vacationSummary(selected);if(vacationChartSelection.year!==selected)vacationChartSelection={year:selected,month:null};
$('vacationAccountRemaining').textContent=`${formatVacationDays(s.remaining)} verfügbar`;$('vacationAccountSelector').classList.toggle('selected',vacationChartSelection.month===null);
const monthData=Array.from({length:12},(_,month)=>{let taken=0,planned=0;for(const r of s.records)for(const d of r.days){if(Number(d.date.slice(5,7))-1!==month)continue;const u=absenceDuration(d)==='half'?0.5:1;if(d.date<=todayKey())taken+=u;else planned+=u}return{month,taken,planned,total:taken+planned}}),max=Math.max(1,...monthData.map(x=>x.total));
$('vacationChart').innerHTML=monthData.map(item=>{const takenHeight=item.taken/max*76,plannedHeight=item.planned/max*76,selectedMonth=vacationChartSelection.month===item.month;return `<button type="button" class="vacation-month ${selectedMonth?'selected':''}" data-vacation-month="${item.month}" aria-label="${['Januar','Februar','März','April','Mai','Juni','Juli','August','September','Oktober','November','Dezember'][item.month]}: ${formatVacationDays(item.taken)} genommen, ${formatVacationDays(item.planned)} geplant"><span class="vacation-bars"><i class="taken" style="height:${takenHeight}px"></i><i class="planned" style="height:${plannedHeight}px"></i></span><b>${['Jan','Feb','Mär','Apr','Mai','Jun','Jul','Aug','Sep','Okt','Nov','Dez'][item.month]}</b></button>`}).join('');
$('vacationChart').querySelectorAll('[data-vacation-month]').forEach(button=>button.addEventListener('click',()=>{vacationChartSelection={year:selected,month:Number(button.dataset.vacationMonth)};renderVacationOverview()}));renderVacationDetail(selected,s,monthData);renderVacationManager();
}
...
/* V5.37 – vereinheitlichte Urlaubs- und Bedienlogik */
let vacationChartSelection={year:new Date().getFullYear(),month:null};
function formatVacationNumber(value){return Number(value||0).toLocaleString('de-DE',{minimumFractionDigits:Number(value)%1?1:0,maximumFractionDigits:1})}
function openAbsenceManager(date=todayKey()){const d=dayObject(date);if(d.absence)openAbsenceEditorForDay(date,d.absenceGroupId&&absenceGroupDays(d.absenceGroupId).length>1?'group':'day');else openNewAbsence('vacation',date)}
function syncAbsenceExistingNotice(){const notice=$('absenceExistingNotice'),date=$('absenceFrom').value,d=dayObject(date),editingDates=new Set(absenceEditorContext?.originalDates||[]),existing=!!(date&&d.absence&&!editingDates.has(date));notice.hidden=!existing;if(existing)$('absenceExistingText').textContent=`${d.absence}${absenceDuration(d)==='half'?' (halber Tag)':''} am ${formatDate(date,{day:'2-digit',month:'2-digit',year:'numeric'})} ist bereits eingetragen.`}
function loadExistingAbsenceFromForm(){const date=$('absenceFrom').value,d=dayObject(date);if(!d.absence)return;openAbsenceEditorForDay(date,d.absenceGroupId&&absenceGroupDays(d.absenceGroupId).length>1?'group':'day')}
function renderTimeCorrectionCard(){const date=$('timeActionDate').value||todayKey(),d=dayObject(date),entries=d.entries||[],future=date>todayKey(),full=hasFullAbsence(d);quickContextDate=date;$('timeActionEntryType').value=nextActionForDay(d);$('timeActionContext').textContent=formatContextDate(date);$('timeActionStatus').textContent=entries.length?`${entries.length} ${entries.length===1?'Buchung':'Buchungen'}`:'Keine Buchung';$('timeActionConflict').hidden=!(future||full);$('timeActionConflict').textContent=future?'Zukünftige Arbeitszeitbuchungen sind nicht zulässig.':full?`Für diesen Tag ist ${d.absence} als ganztägige Abwesenheit eingetragen.`:'';$('timeAddBooking').disabled=future||full;$('timeExistingBookings').innerHTML=entries.length?entries.map((entry,index)=>`<article><div><b>${entry.type==='in'?'Kommen':'Gehen'}</b><span>${esc(entry.logged||entry.actual||'–')}</span><small>Tatsächlich ${esc(entry.actual||'–')} · Dokumentiert ${esc(entry.logged||'–')}</small></div><div><button type="button" data-time-edit="${index}">Bearbeiten</button><button type="button" class="delete" data-time-delete="${index}">Löschen</button></div></article>`).join(''):'<div class="vacation-empty">Für diesen Tag sind keine Arbeitszeitbuchungen vorhanden.</div>';$('timeExistingBookings').querySelectorAll('[data-time-edit]').forEach(button=>button.addEventListener('click',()=>openEntryFromList(date,Number(button.dataset.timeEdit))));$('timeExistingBookings').querySelectorAll('[data-time-delete]').forEach(button=>button.addEventListener('click',()=>deleteTimeEntryFromCard(date,Number(button.dataset.timeDelete))))}
...
document.addEventListener('DOMContentLoaded',()=>{
$('quickCommentToday').addEventListener('click',()=>{closeTodayQuickEntries();openCommentEditor(todayKey(),'direct')});
$('loadExistingAbsenceBtn').addEventListener('click',loadExistingAbsenceFromForm);
$('timeActionDate').addEventListener('change',renderTimeCorrectionCard);$('timeActionEntryType').addEventListener('change',()=>{});$('timeAddBooking').addEventListener('click',()=>openManualTimeQuick($('timeActionDate').value,$('timeActionEntryType').value));$('timeOpenFullDay').addEventListener('click',()=>openFullDayForDate($('timeActionDate').value,'timeActionModal'));
$('vacationAccountSelector').addEventListener('click',()=>{vacationChartSelection={year:Number($('vacationYear').value),month:null};renderVacationOverview()});$('openVacationManager').addEventListener('click',()=>openVacationManagerView());$('vacationManagerYear').addEventListener('change',renderVacationManager);$('vacationManagerAdd').addEventListener('click',()=>openNewAbsence('vacation',`${$('vacationManagerYear').value}-01-01`));
});
...
/* V5.40 / V5.47 – eindeutige Tagesbearbeitung, verbindliche Rundung und ruhige Heute-Seite */
...
function dayCockpitInlineEditor(index){
  ...
  if(!entry)return'';
  return `<div class="day-inline-editor" data-inline-editor="${index}"><div class="day-inline-grid"><div class="field day-inline-type-field"><label for="dayInlineType">Art</label><select id="dayInlineType" data-base="${entry.type}"><option value="in" ${entry.type==='in'?'selected':''}>Kommen</option><option value="out" ${entry.type==='out'?'selected':''}>Gehen</option></select></div><div class="field day-inline-time-field"><label for="dayInlineActual">Tatsächlich</label><input id="dayInlineActual" type="time" value="${esc(entry.actual||'')}"></div><div class="field day-inline-time-field"><label for="dayInlineLogged">Dokumentiert</label><input id="dayInlineLogged" type="time" value="${esc(entry.logged||'')}"></div></div><div class="day-inline-error" id="dayInlineError" hidden></div><div class="day-inline-actions">${adding?'':'<button type="button" class="danger-link" id="dayInlineDelete">Buchung löschen</button>'}<div class="day-inline-action-buttons"><button type="button" id="dayInlineCancel">Abbrechen</button><button type="button" class="primary" id="dayInlineSave">Speichern</button></div></div></div>`;
}
